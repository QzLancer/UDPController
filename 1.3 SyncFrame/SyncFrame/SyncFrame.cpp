// SyncFrame.cpp : 心跳监测指令以1Hz每秒发送，100ms阻塞控制，如果未接收到指令，返回。（是否要多线程？）
// 在没有接受设备的情况下，recvfrom会直接报错，迷惑
// 2022/12/7
// By QzLancer
...
int InitializeUDP(const char* _hostip, u_short _hostport, const char* _controllerip, u_short _controllerport) {
    ...
    tv.tv_sec = 100;
    tv.tv_usec = 0;
    if (setsockopt(HostSocket, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(tv)) < 0) {
...
/// @brief 循环发送同步帧，并捕获回传信息
/// @param _pdustruct 需要发送的PDU结构体
/// @return 如果程序正确，返回0
int SendSyncFrame(PDUStruct _pdustruct) {
    if (_pdustruct.N < 7) {
